...
def get_version():
    with open('cabo/__version__.py') as f:
        for line in f:
...
for entry in glob.glob('cabo/**/*.py', recursive=True):
    if entry.find('__') == 0:
...
setup(name=name,
      ...
      entry_points={'console_scripts': ['server = cabo.server.cli:main',
                                        'client = cabo.client.cli:main']},
      zip_safe=False,
      platforms='any',
      url=url,
      author=author,
